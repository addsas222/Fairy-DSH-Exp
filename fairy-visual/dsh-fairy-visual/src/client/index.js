...
const { VisualSection, MascotSection, IdentitySection } = require('./settings-sections.js');
...
function apply(ctx) {
      ...
      ctx.slots.inject('settings.section', () => ctx.slots.register({ name: 'settings.section', id: 'dsh-fairy-visual', order: 45, label: () => 'HDD 视觉' }, () => jsx(VisualSection, { controller, identitySettings })));
      ctx.slots.inject('settings.section', () => ctx.slots.register({ name: 'settings.section', id: 'dsh-fairy-mascot', order: 46, label: () => '大眼睛主视觉' }, () => jsx(MascotSection, { controller, identitySettings })));
      ctx.slots.inject('settings.section', () => ctx.slots.register({ name: 'settings.section', id: 'dsh-fairy-identity', order: 47, label: () => 'Fairy 身份' }, () => jsx(IdentitySection, { controller, identitySettings })));
      ctx.slots.inject('settings.section', () => ctx.slots.register({ name: 'settings.section', id: 'dsh-fairy-workshop', order: 48, label: () => '创作工坊' }, () => jsx(Workshop, {})));
      }, { surface: 'client' });
    }
