...
test('the settings boundary only accepts known keys and valid levels', async () => {
  ...
  assert.equal(next.timingGate, false, 'a boolean switch is stored');
  assert.equal(next.autoCheck, true, 'and an untouched switch keeps its default');
...
test('the config route writes through and reports what changed', async () => {
  await withHome(async () => {
    ...
    const res = fakeRes();
    await handlers.config(fakeReq({ humanizerLevel: 'l3', memoryImpression: false }), res);
    assert.equal(res.statusCode, 200);
    assert.deepEqual(res.body.changed.sort(), ['humanizerLevel', 'memoryImpression']);
