...
| `.agent-presets/fairy-full/` | 全集 preset（**公开入口**；modes/memory/roleplay 的 agent 面 shim + 语音核心/安全闸门/世界知识三行 + workflow/ralph 与外部 agent 占位行；规则技能见下） |
| `.agent-presets/fairy-lite/` | Fairy Lite preset：人格文本 + 语料（`behavior`/`personality`/`canon`/`style`）+ 三个插件行（纯语音核心变体，不含模式/记忆/角色扮演引擎） |
...
### 4.2 fairy-lite preset 的世界知识（借用官方游戏文本）

`.agent-presets/fairy-lite/` 自带语料（`behavior` 规则 334 KB、`personality` 55 KB、`canon`、`style`）与三个插件行：
...
预设健康检查（发现器的真实判定，避免「能启动但选不中」）：服务运行时 POST `agentPreset.list` 到 `/api/agentPreset.list`，期望 `standard/code/minimal/cordis` 为 `system`、`fairy-full` 与 `fairy-lite` 为 `user`。
...
六项检查：混版检测（同线内版本不一致）、残留安装进程（按**命令行签名**认，不按进程名）、
宿主可运行性（**起新进程跑真二进制**——在跑的宿主是内存态，测不出盘上坏没坏）、
...
- **`fairy-lite` / `fairy-full` 预设选不中**：检查 preset 行名是否为**字面字符串**（发现器的 `entryListProblem` 要求 name 是字符串；`!!js` 会让整个 preset 判 BROKEN），以及 `DSH_FAIRY_REPO_ROOT` 是否导出。
