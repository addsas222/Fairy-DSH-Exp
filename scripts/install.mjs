...
function baseFor(version) {
  // 底座线看**第三段**：0.1.1 → 011、0.1.5 → 015（曾误取第二段，把 0.1.5 推成 011）。
  const patch = Number(/(\d+)\.(\d+)\.(\d+)/.exec(version)?.[3] ?? NaN);
  if (Number.isFinite(patch) && patch >= 5) return '015';
  return '011';
}
...
function installRuntime(options) {
  ...
  const spec = options.installRuntime === 'auto' ? '@deepseek-ai/dsh' : `@deepseek-ai/dsh@${options.installRuntime}`;
...
function writeLauncher(options, runtime, base) {
  ...
  const port = IS_WINDOWS ? '%1' : '${1:-3081}';
  const body = IS_WINDOWS
    ? [
      '@echo off',
      'setlocal',
      'rem 由 scripts/install.mjs 生成（每次部署重写）。仓库里的 scripts/start-fairy.cmd 只是参考副本。',
      `set "PORT=%~1"`,
      ...
      'echo Fairy-DSH  %CD%  port %PORT%  base %DSH_FAIRY_BASE%',
      'node "%RUNTIME%" --profile web --no-open --port %PORT%',
      '',
    ].join('\r\n')
    : [
      '#!/bin/sh',
      '# 由 scripts/install.mjs 生成（每次部署重写）。',
      'set -eu',
      `RUNTIME="${runtime.bin}"`,
      `DSH_FAIRY_BASE="${base}"`,
      `PORT="${port}"`,
  ...
  if (existsSync(launcher)) {
    ...
    if (!options.dryRun) copyFileSync(launcher, `${launcher}.bak`);
...
function main() {
  ...
  if (!runtime && (options.installRuntime || options.runtime === '')) {
    if (!options.installRuntime) {
      warn('没有找到 DSH 运行时；用 --install-runtime 安装一个到 <home>/runtime，或 --runtime 指向已有的');
      fail('缺少运行时（也可以用 DSH_FAIRY_RUNTIME 指定）');
    }
    runtime = installRuntime(options);
  }
